...
public class ProfileController {
    @Inject
    protected AuthorizationFEController authorizationFEController;
    @Inject
...
//        account = accountRepository.findAccountByAccountId(1L);
    ...
    private boolean validate() {
        ...
        accountList = accountRepository.findAllAccountExpertId(authorizationFEController.getAccountDto().getAccountId());
    ...
    public void onSave() {
        if (validate()) {
            ...
            authorizationFEController.setAccountDto(accountDto);
        ...
        if (!newEmail.equals(emailBackup)) {
            ...
            for (Account account : accountList) {
                if (newEmail.equals(account.getEmail())) {
                    ...
                    authorizationFEController.getAccountDto().setEmail(emailBackup);
                    return;
                }
            }
        }
