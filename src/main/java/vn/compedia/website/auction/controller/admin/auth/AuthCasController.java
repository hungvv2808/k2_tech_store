...
package vn.compedia.website.auction.controller.admin.auth;
...
@Getter
@Setter
public class AuthCasController {
    @Inject
    ...
    @Value("${cas.client.admin.login-sso.url}")
            ...
            try {
                ...
                if (account == null) {
                    ...
                    if (account.getRoleId() == DbConstant.ROLE_ID_USER) {
                        throw new Exception("Email " + account.getEmail() + " đã được sử dụng bên trang thông tin");
                    }
                ...
                // process login
                if (!authorizationController.processLogin(account)) {
                    throw new Exception("Quyền hạn không đủ để đăng nhập");
                }
    ...
    private void setErrorForm(String str) {
        ...
        authorizationController.setMESSAGE_ERROR_GROWL(str);
    }
}
