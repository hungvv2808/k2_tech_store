...
import vn.compedia.website.auction.repository.NotifyPaymentRepositoryCustom;
...
select np.notify_payment_id   as 'notify_payment_id',
       np.auction_register_id as 'auction_register_id',
       ...
       ar.code                as 'auction_register_code',
       acc.full_name          as 'full_name',
       a.name                 as 'asset_name',
       r.code                 as 'regulation_code',
       p.payment_id           as 'payment_id'
from notify_payment np
         inner join auction_register ar on np.auction_register_id = ar.auction_register_id
         ...
         left join payment p on ar.auction_register_id = p.auction_register_id
where 1 = 1
...
public class NotifyPaymentRepositoryImpl implements NotifyPaymentRepositoryCustom {
    ...
    public List<NotifyPaymentDto> search(NotifyPaymentSearchDto searchDto) {
        ...
        for (Object[] obj : result) {
            ...
            dto.setAuctionRegisterId(ValueUtil.getLongByObject(obj[1]));
            ...
            dto.setAuctionRegisterCode(ValueUtil.getStringByObject(obj[10]));
    ...
    private void appendQueryFromAndWhereForSearch(StringBuilder sb, NotifyPaymentSearchDto searchDto) {
        ...
        if (searchDto.getAuctionRegisterId() != null) {
            sb.append(" and np.auction_register_id = :auctionRegisterId ");
        }
    ...
    private Query createQueryObjForSearch(StringBuilder sb, NotifyPaymentSearchDto searchDto) {
        ...
        if (searchDto.getAuctionRegisterId() != null) {
            query.setParameter("auctionRegisterId", searchDto.getAuctionRegisterId());
        }
        if (searchDto.getAccountId() != null) {
