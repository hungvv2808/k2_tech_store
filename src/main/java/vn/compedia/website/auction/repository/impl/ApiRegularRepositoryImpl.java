...
import vn.compedia.website.auction.repository.ApiNationRepositoryCustom;
...
@Service
public class ApiRegularRepositoryImpl implements ApiRegularRepositoryCustom {
    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public List<ApiRegular> search(ApiRegularSearchDto searchDto) {
        ...
        sb.append(" GROUP BY n.regular_id ");
        if (searchDto.getSortField() != null) {
            ...
            sb.append(" ORDER BY n.regular_id DESC ");
        }
        ...
        for (Object[] obj : result) {
            ApiRegular dto = new ApiRegular();
            dto.setRegularId(ValueUtil.getLongByObject(obj[0]));
            ...
            apiRegularList.add(dto);
        }
        return apiRegularList;
    }

    @Override
    public  Long countSearch(ApiRegularSearchDto searchDto) {
        StringBuilder sb = new StringBuilder();
        sb.append(" SELECT COUNT(n.regular_id) ");
        appendQueryFromAndWhereForSearch(sb, searchDto);
        Query query = createQueryObjForSearch(sb, searchDto);
        return Long.valueOf(query.getSingleResult().toString());
    }

    private void appendQueryFromAndWhereForSearch(StringBuilder sb, ApiRegularSearchDto searchDto) {
        sb.append(" FROM api_regular n ");
        sb.append(" WHERE 1 = 1 ");
    ...
    private Query createQueryObjForSearch(StringBuilder sb,   ApiRegularSearchDto searchDto) {
    ...
    public void deleteAllRecords() {
        Query query = entityManager.createNativeQuery("DELETE FROM api_regular");
        query.executeUpdate();
    }
}
