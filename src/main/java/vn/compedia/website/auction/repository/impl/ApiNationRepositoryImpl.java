...
@Service
public class ApiNationRepositoryImpl implements ApiNationRepositoryCustom {
    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public List<ApiNation> search(ApiNationSearchDto searchDto) {
        ...
        sb.append(" GROUP BY n.nation_id ");
        if (searchDto.getSortField() != null) {
            ...
            sb.append(" ORDER BY n.nation_id DESC ");
        }
        ...
        for (Object[] obj : result) {
            ApiNation dto = new ApiNation();
            dto.setNationId(ValueUtil.getLongByObject(obj[0]));
    ...
    @Override
    public  Long countSearch(ApiNationSearchDto searchDto) {
        StringBuilder sb = new StringBuilder();
        sb.append(" SELECT COUNT(n.nation_id) ");
        appendQueryFromAndWhereForSearch(sb, searchDto);
        Query query = createQueryObjForSearch(sb, searchDto);
        return Long.valueOf(query.getSingleResult().toString());
    }

    private void appendQueryFromAndWhereForSearch(StringBuilder sb, ApiNationSearchDto searchDto) {
        sb.append(" FROM api_nation n ");
        sb.append(" WHERE 1 = 1 ");
    ...
    private Query createQueryObjForSearch(StringBuilder sb,   ApiNationSearchDto searchDto) {
    ...
    public void deleteAllRecords() {
        Query query = entityManager.createNativeQuery("DELETE FROM api_nation");
        query.executeUpdate();
    }
