...
import vn.compedia.website.auction.dto.common.MailDto;
...
public class EmailUtil implements Runnable {
    ...
    public boolean sendResultAuction(String emailTo, String userName, Boolean status, String regulationCode, String assetName, Date startTime, Date endTime, Long price, Date timeAccept){
        String content;
        if (status == DbConstant.ASSET_MANAGEMENT_ENDING_GOOD) {
            content = PropertiesUtil.getProperty("email.sendResultAuctionSuccess.content")
                    .replace("{USER_NAME}", userName)
            ...
            content = PropertiesUtil.getProperty("email.sendResultAuctionFaile.content")
                    .replace("{USER_NAME}", userName)
        ...
        String subject = PropertiesUtil.getProperty("email.sendResultAuction.subject");
        return mailDtoQueue.add(new MailDto(emailTo, subject, content));
    }
    ...
    public static void main(String[] args) {
      EmailUtil.getInstance().sendResultAuction("thinhhoang3030@gmail.com", "truongvvt", true, "QC000012", "Tài sản A", new Date() , new Date(), 10000000L, new Date());
    }
}
