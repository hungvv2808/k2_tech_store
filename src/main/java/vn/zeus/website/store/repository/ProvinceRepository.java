...
public interface ProvinceRepository extends CrudRepository<Province, Long>, ProvinceRepositoryCustom {
    ...
    @Query("select new vn.zeus.website.store.dto.common.CityDistrictDto(p.name, d.name, c.name) from Province p, District d, Commune c where p.provinceId = d.provinceId and d.districtId = c.districtId and p.provinceId = :provinceId and d.districtId = :districtId and c.communeId = :communeId")
